...
export function LoginPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    e.preventDefault();
    const nextErrors: Record<string, string> = {};
    if (!regName.trim()) nextErrors.regName = 'Full name is required';
    if (!regEmail.trim()) nextErrors.regEmail = 'Email is required';
    if (regEmail && !/^\S+@\S+\.\S+$/.test(regEmail)) nextErrors.regEmail = 'Enter a valid email address';
    if (!regPhone.trim()) nextErrors.regPhone = 'Phone number is required';
    if (regPass.length < 6) nextErrors.regPass = 'Password must be at least 6 characters';
    if (regPass !== regConfirm) {
      nextErrors.regConfirm = "Passwords don't match";
    }
    if (!agreed) nextErrors.agreed = 'You must accept the terms before creating an account';
    setErrors(nextErrors);
    if (Object.keys(nextErrors).length) return;
          ...
          {tab === 'login' ? (
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <button type="button" onClick={() => toast.info('Password reset is not configured for this backend yet')} className="text-xs text-[#1A56DB] hover:underline">Forgot password?</button>
                </div>
                <div className="relative">
              ...
              <button
                type="button"
                onClick={() => toast.info('Google sign-in is not configured for this backend yet')}
            ...
            <form onSubmit={handleRegister} className="space-y-4">
              <div>
                ...
                {errors.regName && <p className="text-xs text-red-600 mt-1">{errors.regName}</p>}
              </div>
              <div>
                <label className="block text-sm text-gray-700 mb-1">Email</label>
                <div className="relative">
                ...
                {errors.regEmail && <p className="text-xs text-red-600 mt-1">{errors.regEmail}</p>}
              </div>
              <div>
                ...
                {errors.regPhone && <p className="text-xs text-red-600 mt-1">{errors.regPhone}</p>}
              </div>
              <div>
                ...
                {errors.regPass && <p className="text-xs text-red-600 mt-1">{errors.regPass}</p>}
                {regPass && (
              ...
              <div>
                ...
                {errors.regConfirm && <p className="text-xs text-red-600 mt-1">{errors.regConfirm}</p>}
              </div>
